...
network_2=NetworkReader.readFrom(net_fold+ 'network_Type2H1_Both100.xml')
network_4=NetworkReader.readFrom(net_fold+ 'network_Type2H2_Both100.xml')
...
sim_deNorm2=np.column_stack((de_standardizer(de_normalizer(sim_net2[:,0],standardizer(theta_Val[:,0])),theta_Val[:,0]),de_standardizer(de_normalizer(sim_net2[:,1],standardizer(theta_Val[:,1])),theta_Val[:,1]),de_standardizer(de_normalizer(sim_net2[:,2],standardizer(theta_Val[:,2])),theta_Val[:,2]),de_standardizer(de_normalizer(sim_net2[:,3],standardizer(theta_Val[:,3])),theta_Val[:,3]),de_standardizer(de_normalizer(sim_net2[:,4],standardizer(theta_Val[:,4])),theta_Val[:,4])))
...
#save
beta.to_csv(save_fold+'Lutein_NN_Type2H1_CrossVal_Both100_ExpTraj.csv')
theta.to_csv(save_fold+'Lutein_NN_Type2H1_CrossVal_Both100_ExpTraj_Rescale.csv')
#overwrite
beta=pd.DataFrame(arr_valT,columns=lista_col2)
beta.drop('Time',axis=1, inplace=True)
theta=pd.DataFrame(arr_temp[len(arr_test):],columns=lista_col2)
...
#denormalize and rescale to real size
sim_deNorm4=np.column_stack((de_standardizer(de_normalizer(sim_net4[:,0],standardizer(theta_Val[:,0])),theta_Val[:,0]),de_standardizer(de_normalizer(sim_net4[:,1],standardizer(theta_Val[:,1])),theta_Val[:,1]),de_standardizer(de_normalizer(sim_net4[:,2],standardizer(theta_Val[:,2])),theta_Val[:,2]),de_standardizer(de_normalizer(sim_net4[:,3],standardizer(theta_Val[:,3])),theta_Val[:,3]),de_standardizer(de_normalizer(sim_net4[:,4],standardizer(theta_Val[:,4])),theta_Val[:,4])))
...
#save
beta.to_csv(save_fold+'Lutein_NN_Type2H2_CrossVal_Both100_ExpTraj.csv')
theta.to_csv(save_fold+'Lutein_NN_Type2H2_CrossVal_Both100_ExpTraj_Rescale.csv')
#overwrite
beta=pd.DataFrame(arr_valT,columns=lista_col2)
beta.drop('Time',axis=1, inplace=True)
theta=pd.DataFrame(arr_temp[len(arr_test):],columns=lista_col2)
...
theta=pd.concat([theta,temp2a],axis=1,keys=['Experimental','Simulated'])     
#save
beta.to_csv(save_fold+'Lutein_NN_Type2H1_CrossVal_Both100_SimTraj.csv')
theta.to_csv(save_fold+'Lutein_NN_Type2H1_CrossVal_Both100_SimTraj_Rescale.csv')
#overwrite
beta=pd.DataFrame(arr_valT,columns=lista_col2)
beta.drop('Time',axis=1, inplace=True)
theta=pd.DataFrame(arr_temp[len(arr_test):],columns=lista_col2)
...
theta=pd.concat([theta,temp4a],axis=1,keys=['Experimental','Simulated'])   
#save
beta.to_csv(save_fold+'Lutein_NN_Type2H2_CrossVal_Both100_SimTraj.csv')
theta.to_csv(save_fold+'Lutein_NN_Type2H2_CrossVal_Both100_SimTraj_Rescale.csv')
#overwrite
beta=pd.DataFrame(arr_valT,columns=lista_col2)
beta.drop('Time',axis=1, inplace=True)
theta=pd.DataFrame(arr_temp[len(arr_test):],columns=lista_col2)
