...
#save
alpha.to_csv(save_fold+'Lutein_NN_Type2H1_TestCross30.csv')
...
#save
alpha.to_csv(save_fold+'Lutein_NN_Type2H2_TestCross30.csv')
#overwrite
alpha=pd.DataFrame(array_data,columns=lista_col1)
alpha.drop('Time',axis=1, inplace=True)

#simulation for Network 1, 1 Hidden.
#This type of network takes 5 inputs, including the time difference.
#1 hidden layer
#go through the different time differences
#A note: Enumerate produces a tuple from a list that goes (index of item A, list item A)
for dT in enumerate(deltaT):
    for k in [0,13,26]:
        #test different test sets
        sim_net1=np.array([np.append([array_data[k,0]],array_data[k,2:])])
        #simulation for Network 1, 1 Hidden.
        for j in enumerate(xrange(0,12,intrv[dT[0]])):
            ans=network_1.activate((sim_net1[j[0]][0],dT[1],sim_net1[j[0]][1],sim_net1[j[0]][2],sim_net1[j[0]][3]))
            ansK=[array_data[k,0],sim_net1[j[0]][1]+ans[0],sim_net1[j[0]][2]+ans[1],sim_net1[j[0]][3]+ans[2]]
            sim_net1=np.append(sim_net1,[ansK],axis=0)
        if k==0 :
            #This needs to be created as a data frame thbe first time around
            temp1A=pd.DataFrame(sim_net1,columns=lista_col2)
        else:
            #This needs to be concatenated after that
...
alpha.to_csv(save_fold+'Lutein_NN_Type1H1_TestCross30.csv')
#overwrite
alpha=pd.DataFrame(array_data,columns=lista_col1)
alpha.drop('Time',axis=1, inplace=True)
...
for dT in enumerate(deltaT):
    for k in [0,13,26]:
        ...
        #Since the time differences are larger and larger, fewer points need to be simulated
        #So the step trhough the range increases as we get largers dT
        for j in enumerate(xrange(0,12,intrv[dT[0]])):
            ans=network_3.activate((sim_net3[j[0]][0],dT[1],sim_net3[j[0]][1],sim_net3[j[0]][2],sim_net3[j[0]][3]))
            ansK=[array_data[k,0],sim_net3[j[0]][1]+ans[0],sim_net3[j[0]][2]+ans[1],sim_net3[j[0]][3]+ans[2]]
...
alpha=pd.concat([alpha,temp1],axis=1)    
alpha.to_csv(save_fold+'Lutein_NN_Type1H2_TestCross30.csv')
...
#save
beta.to_csv(save_fold+'Lutein_NN_Type2H1_CrossVal30.csv')
#overwrite
beta=pd.DataFrame(array_data,columns=lista_col1)
beta.drop('Time',axis=1, inplace=True)
...
#save
beta.to_csv(save_fold+'Lutein_NN_Type2H2_CrossVal30.csv')
#overwrite
beta=pd.DataFrame(array_data,columns=lista_col1)
beta.drop('Time',axis=1, inplace=True)
...
for dT in enumerate(deltaT):
    ...
    #simulation for Network 1, 1 Hidden.
    for j in enumerate(xrange(0,12,intrv[dT[0]])):
        ans=network_1.activate((cross_arr[j[0]][0],dT[1],cross_arr[j[0]][1],cross_arr[j[0]][2],cross_arr[j[0]][3]))
        ansK=[cross_arr[0,0],cross_arr[j[0]][1]+ans[0],cross_arr[j[0]][2]+ans[1],cross_arr[j[0]][3]+ans[2]]
        sim_net1=np.append(sim_net1,[ansK],axis=0)
    temp1A=pd.DataFrame(sim_net1,columns=lista_col2)
    if dT[0]==0:
        temp1=temp1A
...
beta.to_csv(save_fold+'Lutein_NN_Type1H1_CrossVal30.csv')
#overwrite
beta=pd.DataFrame(array_data,columns=lista_col1)
beta.drop('Time',axis=1, inplace=True)
...
for dT in enumerate(deltaT):
    sim_net3=np.array([np.append([cross_arr[0,0]],cross_arr[0,2:])])
        #simulation for Network 1, 2 Hidden.
    for j in enumerate(xrange(0,12,intrv[dT[0]])):
        ans=network_3.activate((cross_arr[j[0]][0],dT[1],cross_arr[j[0]][1],cross_arr[j[0]][2],cross_arr[j[0]][3]))
        ansK=[cross_arr[0,0],cross_arr[j[0]][1]+ans[0],cross_arr[j[0]][2]+ans[1],cross_arr[j[0]][3]+ans[2]]
        sim_net3=np.append(sim_net3,[ansK],axis=0)
    temp3A=pd.DataFrame(sim_net3,columns=lista_col2)
    if dT[0]==0:
        ...
        temp3=pd.concat([temp3,temp3A],axis=1)
beta=pd.concat([beta,temp1],axis=1)
beta.to_csv(save_fold+'Lutein_NN_Type1H2_CrossVal30.csv')