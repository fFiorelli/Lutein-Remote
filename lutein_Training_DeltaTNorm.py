...
for stp in enumerate(time_int):
    for j in r_lengths:
        ...
        for k in xrange(0,len(temp_data)-1,stp[0]+1):
            ds2.addSample((temp_data[k][0],temp_data[k][1],temp_data[k][2],temp_data[k][3],temp_data[k][4],stp[1]), (temp_data[k+1][1]-temp_data[k][1],temp_data[k+1][2]-temp_data[k][2],temp_data[k+1][3]-temp_data[k][3]))
...
NetworkWriter.writeToFile(net2,net_fold+ 'network_Type2H1_DeltaT.xml')
trainer4 = BackpropTrainer(net4, ds2, verbose = True)
trainer4.trainEpochs(nEpochs)
NetworkWriter.writeToFile(net4,net_fold+ 'network_Type2H2_DeltaT.xml')
