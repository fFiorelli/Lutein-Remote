...
enh_array=np.column_stack((normalizer(standardizer(enh_array[1:,0])),normalizer(standardizer(enh_array[1:,2])),normalizer(standardizer(enh_array[1:,3])),normalizer(standardizer(enh_array[1:,4])),normalizer(standardizer(enh_array[1:,5]))))


    #build and populate datasets
ds2 = SupervisedDataSet(5, 3)
    #this is meant to 
r_lengths=[[i,i+12] for i in xrange(0,12*50*5,12)]
    #print r_lengths[-1]
    #print len(enh_array)
...
NetworkWriter.writeToFile(net2,net_fold+ 'network_Type2H1_Both_Less.xml')
trainer4 = BackpropTrainer(net4, ds2, verbose = True)
trainer4.trainEpochs(nEpochs)
NetworkWriter.writeToFile(net4,net_fold+ 'network_Type2H2_Both_Less.xml')
