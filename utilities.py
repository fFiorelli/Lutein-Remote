...
#this attempts to denormalize based on the original list
def de_normalizer(var_list, org_var):
    #applies the Standard Score
    test_list=[]
    test_list=np.array(org_var,dtype='float64')
    ...
    test_Max=max(test_list)
    for j in xrange(0,len(var_list)):
        test_list_2.append((var_list[j])*(test_Max-test_Min)+test_Min)

    return np.array(test_list_2,dtype='float64')

def de_standardizer(var_list, org_var):
    #applies the Standard Score
    test_list=[]
    test_list=np.array(org_var,dtype='float64')
    ...
    if test_dev > 0.00001:
        for j in xrange(0,len(var_list)):    
            test_list_2.append((float(var_list[j]))*test_dev +test_mean)
    else:
        test_list_2=np.zeros(len(test_list))
        print 'Zero Exception activated'
