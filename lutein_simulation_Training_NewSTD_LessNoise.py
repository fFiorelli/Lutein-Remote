...
arr_testT2=np.column_stack((standardizer(arr_test[:,0]),normalizer(standardizer(arr_test[:,1])),standardizer(arr_test[:,2]),standardizer(arr_test[:,3]),standardizer(arr_test[:,4]),standardizer(arr_test[:,5])))
beta=pd.DataFrame(arr_testT2,columns=lista_col2)
beta.drop('Time',axis=1, inplace=True)
#theta is the pre-treatment dataframe
theta=pd.DataFrame(arr_test,columns=lista_col2)
...
sim_net2=np.append(sim_net2,[beta_Val[24]],axis=0)
print len(sim_net2)
for j in xrange(24,35):
    ans=network_2.activate((sim_net2[j]))
    ansK=[beta_Val[j,0],sim_net2[j][1]+ans[0],sim_net2[j][2]+ans[1],sim_net2[j][3]+ans[2],beta_Val[j+1][4]]
    sim_net2=np.append(sim_net2,[ansK],axis=0)

sim_net2=np.append(sim_net2,[beta_Val[36]],axis=0)
for j in xrange(36,47):
    ans=network_2.activate((sim_net2[j]))
    ansK=[beta_Val[j,0],sim_net2[j][1]+ans[0],sim_net2[j][2]+ans[1],sim_net2[j][3]+ans[2],beta_Val[j+1][4]]
    sim_net2=np.append(sim_net2,[ansK],axis=0)

sim_net2=np.append(sim_net2,[beta_Val[48]],axis=0)
print len(sim_net2)
for j in xrange(48,59):
    ans=network_2.activate((sim_net2[j]))
    ansK=[beta_Val[j,0],sim_net2[j][1]+ans[0],sim_net2[j][2]+ans[1],sim_net2[j][3]+ans[2],beta_Val[j+1][4]]
    sim_net2=np.append(sim_net2,[ansK],axis=0)
print len(sim_net2)
...
#save
beta.to_csv(save_fold+'Lutein_NN_Type2H1_Training_LessNoise2_SimTrajSTD.csv')
theta.to_csv(save_fold+'Lutein_NN_Type2H1_Training_LessNoise2_SimTrajSTD_Rescale.csv')
#overwrite
beta=pd.DataFrame(arr_testT2,columns=lista_col2)
beta.drop('Time',axis=1, inplace=True)
theta=pd.DataFrame(arr_test,columns=lista_col2)
...
sim_net4=np.append(sim_net4,[beta_Val[24]],axis=0)
for j in xrange(24,35):
    ans=network_4.activate((sim_net4[j]))
    ansK=[beta_Val[j,0],sim_net4[j][1]+ans[0],sim_net4[j][2]+ans[1],sim_net4[j][3]+ans[2],beta_Val[j+1][4]]
    sim_net4=np.append(sim_net4,[ansK],axis=0)

sim_net4=np.append(sim_net4,[beta_Val[36]],axis=0)
for j in xrange(36,47):
    ans=network_4.activate((sim_net4[j]))
    ansK=[beta_Val[j,0],sim_net4[j][1]+ans[0],sim_net4[j][2]+ans[1],sim_net4[j][3]+ans[2],beta_Val[j+1][4]]
    sim_net4=np.append(sim_net4,[ansK],axis=0)

sim_net4=np.append(sim_net4,[beta_Val[48]],axis=0)
for j in xrange(48,59):
    ans=network_4.activate((sim_net4[j]))
    ansK=[beta_Val[j,0],sim_net4[j][1]+ans[0],sim_net4[j][2]+ans[1],sim_net4[j][3]+ans[2],beta_Val[j+1][4]]
    sim_net4=np.append(sim_net4,[ansK],axis=0)
...
#save
beta.to_csv(save_fold+'Lutein_NN_Type2H2_Training_LessNoise2_SimTrajSTD.csv')
theta.to_csv(save_fold+'Lutein_NN_Type2H2_Training_LessNoise2_SimTrajSTD_Rescale.csv')
#overwrite
beta=pd.DataFrame(arr_testT2,columns=lista_col2)
beta.drop('Time',axis=1, inplace=True)
theta=pd.DataFrame(arr_test,columns=lista_col2)
